...
void _O2 Endstops::report_states() { 
  ...
  #define ES_REPORT(S) print_es_state(READ(S##_PIN) != S##_ENDSTOP_INVERTING, PSTR(STR_##S))
	...
	if(bltouch.enable_state){				
		...
		print_es_state(READ(Z_MIN_PIN) == Z_MIN_ENDSTOP_INVERTING, PSTR(STR_Z_MIN));
	}
	#else
	ES_REPORT(Z_MIN);
	#endif
...
void Endstops::update() {
   ...
   */
	...
	if(bltouch.enable_state){		
		...
		SET_BIT_TO(live_state, _ENDSTOP(Z, MIN), (READ(_ENDSTOP_PIN(Z, MIN)) == _ENDSTOP_INVERTING(Z, MIN)));
	}
	#else
	UPDATE_ENDSTOP_BIT(Z, MIN);
	#endif
