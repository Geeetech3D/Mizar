...
#include "../core/debug_out.h"
  ...
  #define EEPROM_FINISH()         do{ DISABLE_ISRS(); persistentStore.access_finish(); ENABLE_ISRS();}while(0)
  ...
  bool MarlinSettings::save() {
    ...
    //
    {
      ...
      const bool bltouch_enable_state = TERN(BLTOUCH, bltouch.enable_state, false);
  ...
  bool MarlinSettings::_load() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      //
      {
        #if ENABLED(BLTOUCH_MENU_OPTION)
      	_FIELD_TEST(bltouch_enable_state);
        const bool &bltouch_enable_state = bltouch.enable_state;
