...
void lv_draw_filament_change(void) {
  ...
  disp_filament_temp();
	...
	lv_obj_set_style(filamentWarn, &tft_style_label_rel);
	lv_obj_set_pos(filamentWarn, 165, 140);
	disp_filament_warn();
}
...
void disp_filament_warn() {
	...
	if (printingIsActive()) {
		if(!flash_switch){
			flash_switch = 1;
			lv_label_set_text(filamentWarn,  filament_menu.ready_replace);
		}else{
			flash_switch = 0;
			lv_label_set_text(filamentWarn, buf);
		}
	}else{
		lv_label_set_text(filamentWarn, buf);
	}
