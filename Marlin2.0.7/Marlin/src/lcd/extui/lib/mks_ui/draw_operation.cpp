...
void lv_draw_operation(void) {
  ...
  if (uiCfg.print_state != WORKING) {
    ...
    buttonBabyStep = lv_imgbtn_create(scr, NULL);
		if (bltouch.enable_state == 0) 
			lv_obj_set_hidden(buttonBabyStep, 1);
		#endif
  }
  ...
  lv_imgbtn_set_style(buttonFilament, LV_BTN_STATE_REL, &tft_style_label_rel);
    ...
    if (uiCfg.print_state != WORKING) {
      ...
      //lv_imgbtn_set_src(buttonExtrusion, LV_BTN_STATE_REL, "F:/bmp_extruct.bin");//"F:/bmp_Extruct_opr.bin");
      //lv_imgbtn_set_src(buttonExtrusion, LV_BTN_STATE_PR, "F:/bmp_clear.bin");//"F:/bmp_Extruct_opr.bin");
      ...
      //lv_imgbtn_set_src(buttonMove, LV_BTN_STATE_REL, "F:/bmp_mov_sel.bin");//"F:/bmp_move_opr.bin");
      //lv_imgbtn_set_src(buttonMove, LV_BTN_STATE_PR, "F:/bmp_clear.bin");//"F:/bmp_move_opr.bin");
  ...
  /*
    ...
    */
      //lv_obj_set_pos(buttonExtrusion,  BTN_X_PIXEL * 3 + INTERVAL_V * 4, titleHeight);
      //lv_obj_set_pos(buttonMove, INTERVAL_V, BTN_Y_PIXEL + INTERVAL_H + titleHeight);
      lv_obj_set_pos(buttonBack, BTN_X_PIXEL * 3 + INTERVAL_V * 4, titleHeight);
    }
    else {
      lv_obj_set_pos(buttonSpeed,  BTN_X_PIXEL * 3 + INTERVAL_V * 4, titleHeight);
			#if ENABLED(BLTOUCH)
      lv_obj_set_pos(buttonBabyStep, INTERVAL_V, BTN_Y_PIXEL + INTERVAL_H + titleHeight);
			#endif
			lv_obj_set_pos(buttonBack, BTN_X_PIXEL * 3 + INTERVAL_V * 4, BTN_Y_PIXEL + INTERVAL_H + titleHeight);
  }
  #endif
