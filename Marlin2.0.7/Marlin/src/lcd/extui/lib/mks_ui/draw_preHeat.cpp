...
static lv_obj_t *hotendTempText, *bedTempText;
...
void lv_draw_preHeat(void) {
  ...
  lv_imgbtn_set_src(buttonCool, LV_BTN_STATE_REL, "F:/bmp_cool.bin");
  ...
  lv_imgbtn_set_style(buttonCool, LV_BTN_STATE_REL, &tft_style_label_rel);
	...
	lv_obj_set_pos(buttonCool, BTN_X_PIXEL * 3 + INTERVAL_V * 4, titleHeight);
  ...
  lv_obj_set_pos(buttonBAS, BTN_X_PIXEL + INTERVAL_V * 2, BTN_Y_PIXEL + INTERVAL_H + titleHeight);
	lv_obj_set_pos(buttonCUS, BTN_X_PIXEL * 2 + INTERVAL_V * 3, BTN_Y_PIXEL + INTERVAL_H + titleHeight);
...
void disp_preHeat_temp() {
	...
	lv_label_set_text(hotendTempText, public_buf_l);
	lv_obj_align(hotendTempText, NULL, LV_ALIGN_CENTER, 0, -80);
	...
	strcat_P(public_buf_l, PSTR(": "));
	strcat(public_buf_l, buf);
	lv_label_set_text(bedTempText, public_buf_l);
	lv_obj_align(bedTempText, NULL, LV_ALIGN_CENTER, 0, -50);
}
